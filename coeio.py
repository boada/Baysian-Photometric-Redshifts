...
def savedata(data,
    ...
    if dow and os.path.exists(outfile):
        ...
        colneg = [
            0
        ] * nx  # WHETHER THE COLUMN HAS ANY NEGATIVE NUMBERS: 1=YES, 0=NO
        collens = []
        if format:
            ...
            colint = [
                0
            ] * nx  # LENGTH BEFORE DECIMAL PLACE (INCLUDING AN EXTRA ONE IF IT'S NEGATIVE)
            #colneg = [0] * nx  # WHETHER THE COLUMN HAS ANY NEGATIVE NUMBERS: 1=YES, 0=NO
            colexp = [
                0
            ] * nx  # WHETHER THE COLUMN HAS ANY REALLY BIG NUMBERS THAT NEED exp FORMAT : 1=YES, 0=NO
                ...
                for ix in range(nx):
                    ...
                    if ni > colint[ix]:  # IF BIGGEST, YOU GET TO DECIDE NEG SPACE OR NO
                        ...
                        #print '>', ix, colneg[ix], nd, coldec[ix]
                            ...
                            ix]:  # IF MATCH BIGGEST, YOU CAN SET NEG SPACE ON (NOT OFF)
            ...
            for ix in range(nx):
                if colexp[ix]:
                    ...
                    collen = colint[ix] + coldec[ix] + (coldec[ix] > 0) + (
                        colneg[ix] > 0
                    )  # EXTRA ONES FOR DECIMAL POINT / - SIGN
                if labels and not machine:
                    collen = max(
                        (collen, len(labels[ix])
            ...
            if not collens:
                ...
                for ix in range(nx):
                    ...
                    if string.find(cf, 'e') > -1:
                        ...
                        cf = string.split(cf,
                                          '.')[0]  # FLOAT: Number before '.'
                        cf = string.split(cf,
                                          'd')[0]  # INT:   Number before 'd'
                        collen = string.atoi(cf)
                    if labels:
                        collen = max(
                            (collen, len(labels[ix])
...
class VarsClass(object):
    ...
    def subset(self, good):
        ...
        if self.len() != len(good):
            ...
            selfcopy.assigndata()
            selfcopy.data = compress(
                good, self.data)  # PRESERVE UN-UPDATED DATA ARRAY
    ...
    def append(self, self2, over=0):
        ...
        if labels != labels2:
            ...
            for label in self.labels:
                exec(
                    'self.%s = concatenate((self.get(label), self2.get(label)))'
                    % label)
            self.updatedata()
        return self
    ...
    def savefitstable(self,
                      ...
                      format={},
                      labels=1,
                      overwrite=1):  # FITS TABLE
        name = name or recapfile(self.name,
...
def loadvarswithclass(filename,
                      dir="",
                      silent=0,
                      labels='',
                      header='',
...
def loadsexcat(infile,
    ...
    for headline in header:
        ...
        if param[:4] == "MAG_":  # MAG_AUTO or MAG_APER but not MAGERR_AUTO
            ...
            if (
                    param == magname
            ) or not magname:  # magname IF YOU ONLY WANT MAG_AUTO (DEFAULT)
                magname = param
                param = "MAG"
        if param[:7] == "MAGERR_":  # MAGERR_AUTO or MAGERR_APER
            ...
            if (
                    param == magerrname
            ) or not magerrname:  # magname IF YOU ONLY WANT MAG_AUTO (DEFAULT)
                magerrname = param
                param = "MAGERR"
...
# ASSIGN COLUMN(S), NOW THAT WE KNOW HOW MANY THERE ARE
    ...
    for ii in range(len(params)):
        ...
        param = paramtranslate.get(
            param,
            param)  # CHANGE IT IF IN DICTIONARY, OTHERWISE LEAVE IT ALONE
    ...
    if 'imaflagsiso' in params:
        ...
        flags = bitwise_or(
            flags.astype(int),
            imaflagsiso.astype(int))  # "FLAGS" OR "IMAFLAGS_ISO"
...
def savelabels(x,
    ...
    fout = open(filename, 'w')
    fout.write(
        'global color=%s font="helvetica %d %s" select=1 edit=1 move=1 delete=1 include=1 fixed=0 source\n'
